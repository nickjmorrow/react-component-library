...
const SideNavInternal: React.SFC<Props & RouteComponentProps> = ({
    ...
    const { colors, transitions, spacing, boxShadow, defaultShowBoxShadow } = React.useContext(ThemeContext);

    // TODO: naming
    const finalShowBoxShadow = showBoxShadow === undefined ? defaultShowBoxShadow : showBoxShadow;
    ...
    const renderNavLink = (
        ...
        return (
            ...
            >
                <Link
                    route={navLink.route}
                    styleVariant={currentRoute === navLink.route ? 1 : 2}
...
const FolderLabel: React.SFC<{ spacing: StyleConstant<'spacing'> }> = ({ spacing, children }) => (
...
const Folder: React.SFC<{
    ...
    renderNavLink: (navInfo: INavLink, marginLeft: string, i: number, handleLinkClick: () => void) => React.ReactNode;
    currentRoute: string;
    style?: React.CSSProperties;
            ...
            <>
                ...
                <Collapse isOpened={isExpanded} springConfig={{ stiffness: 220 }}>
                    <div ref={innerNavLinkRef}>
                        {folderInfo.navLinks.map((nl, i) => renderNavLink(nl, spacing.ss12, i, handleLinkClick))}
                    </div>
                </Collapse>
            </>
        );
    ...
    (prevProps, nextProps) => {
        const doesFolderContainChangingLink = (
            ...
            folderInfo.navLinks.some(
                nl => nl.route === folderNextProps.currentRoute || nl.route === folderPrevProps.currentRoute,
            );

        return (
            equal(prevProps.folderInfo, nextProps.folderInfo) &&
            !doesFolderContainChangingLink(nextProps.folderInfo, prevProps, nextProps) &&
