...
export const LabeledCheckbox: React.SFC<{
    ...
    ({ isToggled, option, onClick: handleClick }) => {
        ...
        const renderLabel = () => (
            <Typography colorVariant={isToggled ? 'core' : 'primaryDark'}>{option.label}</Typography>
        );
        return (
            <LabeledInputWrapper onClick={handleClickInternal} renderInput={renderInput} renderLabel={renderLabel} />
        );
    },
